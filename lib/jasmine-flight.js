...
(function (root) {
   ...
   * @param target {Object} (Optional) Target jQuery object to which the component should be attached
   */

  root.setupComponent = function (fixture, options, target) {
      jasmine.getEnv().currentSpec.setupComponent(fixture, options, target);
  };

  jasmine.Spec.prototype.setupComponent = function (fixture, options, target) {
    if (this.component) {
    ...
    if (typeof target !== 'undefined') {
      target.addClass('component-root');
      this.$node.removeClass('component-root');
    } else {
      target = this.$node;
    }

    this.component = (new this.Component()).initialize(target, options);
  };
